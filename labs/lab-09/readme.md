...
> [!NOTE]
> How do you know when to stop walking? Think about the next string that comes *after* all
> words with this prefix lexicographically. The prefix `"pre"` ends before `"prf"` — stop
> as soon as the iterator reaches that boundary. `std::priority_queue` is a max-heap by
> default; a pair `{int, string}` compares on the first element first, which is exactly
> what you want.
...
| Container | Header | Role |
|-----------|--------|------|
| `std::list<std::pair<std::string, std::vector<std::string>>>` | `<list>` | Doubly-linked; MRU entry at front, LRU at back |
...
### Running Part 2 Standalone

Once you have implemented `LRUCache` and wired it into `main()`, compile only `lru.cpp` — it already pulls in `autocomplete.cpp` internally:
...
> uses `#include "autocomplete.cpp"` internally, so compiling both would produce duplicate
> symbol errors.

## Testing Your Code

`unittester.cpp` is an automated test harness that compiles both of your source files and runs 15 checks — one per function behavior. Each test reports `[PASS]` or `[FAIL]` and its point value. Your score out of 40 is printed at the end.
...
### Step 1 — Compile the test harness

```bash
$ g++ -std=c++17 -Wall -Werror -DTESTING -o tests unittester.cpp
```

The `-DTESTING` flag tells the compiler to skip the `main()` functions inside `autocomplete.cpp` and `lru.cpp` so the test harness can supply its own. You must recompile every time you change your code.
